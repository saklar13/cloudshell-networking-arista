...
class SNMPEntityTable(object):
    ...
    def _get_sorted_modules_with_ports(self):
        ...
        self._sorted_module_list = [module for module in module_relative_paths
                                    if module in self._module_list
                                    and module not in self.exclusion_list]
    ...
    def _get_entity_table(self):
        ...
        for index in physical_indexes.keys():
            ...
            temp_entity_table.update(
                self._snmp.get_properties('ENTITY-MIB', index, entity_table_critical_port_attr)[index])

            vendor_type = temp_entity_table['entPhysicalVendorType'].lower()

            if vendor_type in ('cevsensor', 'cevfan'):
                continue
            ...
            if temp_entity_table['entPhysicalClass'] == '' or "other" in temp_entity_table['entPhysicalClass']:
                index_entity_class = self.ENTITY_VENDOR_TYPE_TO_CLASS_MAP.get(vendor_type)
                if index_entity_class:
                    temp_entity_table['entPhysicalClass'] = index_entity_class
                else:
                    continue

            if re.search(self.entity_to_container_pattern, vendor_type, re.IGNORECASE):
            ...
            temp_entity_table.update(
                self._snmp.get_properties('ENTITY-MIB', index, entity_table_optional_port_attr)[index])
    ...
    def _filter_entity_table(self, raw_entity_table):
        ...
        for element in reversed(elements):
            ...
            if ((parent_id not in raw_entity_table
                 and "chassis" not in raw_entity_table[element]["entPhysicalClass"])
                    or parent_id in self.exclusion_list):
