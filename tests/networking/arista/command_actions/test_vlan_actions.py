...
from cloudshell.networking.arista.command_actions.add_remove_vlan_actions import \
    AddRemoveVlanActions
from cloudshell.networking.arista.command_templates.add_remove_vlan import L2_TUNNEL
...
Boogie#
"""
        self.assertIsNotNone(self._handler.verify_interface_configured(vlan_id, current_config))
        self.assertIsNone(self._handler.verify_interface_configured(vlan_id_2, current_config))
        self.assertIsNone(self._handler.verify_interface_configured(vlan_id_3, current_config))

    @patch("cloudshell.networking.arista.command_actions.add_remove_vlan_actions."
    ...
    def test_get_l2_protocol_tunnel_cmd(self, cte_mock):
        ...
        cte_mock.assert_called_once_with(self._cli_service, L2_TUNNEL,
    ...
    def test_config_vlan_on_interface_qnq(self, cte_mock):
        ...
        cte_mock.side_effect = [
            conf_interface_mock, no_shut_mock, enable_switchport_mock,
            switchport_mode_mock, MagicMock(), switchport_allow_vlan_mock
        ]

        self._handler.set_vlan_to_interface(
            vlan_range=vlan_range, port_name=port_name,
            port_mode=port_mode, qnq=True, c_tag=None)
    ...
    def test_config_vlan_on_interface_no_qnq_access(self, cte_mock):
        ...
        cte_mock.side_effect = [
            conf_interface_mock, no_shut_mock, enable_switchport_mock,
            switchport_mode_mock, switchport_allow_vlan_mock]

        self._handler.set_vlan_to_interface(
            vlan_range=vlan_range, port_name=port_name,
            port_mode=port_mode, qnq=False, c_tag=None)
